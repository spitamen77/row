...
$this->title = Uni::t('app', 'Hudud');
\app\components\widgets\SweetAlertAsset::register($this);
$this->registerJs('Muxr.saveHudud();Muxr.openClearedDirection();Muxr.openDirectionForm();Muxr.openEditDirectionForm();Muxr.openDeleteDirectionForm();Muxr.editHududStatus();');
        ...
        <div class="uk-grid">
            <div class="uk-width-1-2">
                <? if (Uni::$app->controller->access("ADMIN_TUM")) { ?>
                    <a class="md-btn md-btn-success md-btn-small" href="<?=Url::to('hr/default/index')?>"><?=Uni::t('app','Back')?></a>
                <? }else{ ?>
                    <a class="md-btn md-btn-success md-btn-small" href="<?=Url::to('reference/default/index')?>"><?=Uni::t('app','Back')?></a>
                <? } ?>
                
                <a class="md-btn md-btn-success md-btn-small" href="<?=Url::to('reference/hudud/index')?>"><?=Uni::t('app','All')?></a>
                <button id="modal_add_direction_btn" data-uk-modal="{bgClose:true;modal:true}" class="md-btn md-btn-facebook md-btn-small" ><?=Uni::t('app', 'Add hudud')?></button>
<!--                <div class="uk-button-dropdown" data-uk-dropdown="{mode:'click'}">-->
<!--                    <button class="md-btn md-btn-block md-btn-small">--><?//=Uni::t('app', 'Regions')?><!-- <i class="material-icons"></i></button>-->
<!--                    <div class="uk-dropdown">-->
<!--                        <ul class="uk-nav uk-nav-dropdown">-->
<!--                            <li><a href="--><?//=Url::to('reference/hudud/index')?><!--" class="viloyat">--><?//= Uni::t('app',"All") ?><!--</a></li>-->
<!--                            --><?php //foreach ($items as $item): ?>
<!--                                <li><a href="--><?//=Url::to('reference/hudud/index?slug=item&region='.$item->id)?><!--" class="viloyat" data-id="--><?//=$item->id?><!--">--><?//= ($item->name)?$item->name:Uni::t('app',"Not set") ?><!--</a></li>-->
<!--                            --><?php //endforeach; ?>
<!--                        </ul>-->
<!--                    </div>-->
<!--                </div>-->
        ...
        <div class="md-card-content">
            <table class="uk-table uk-table-nowrap table_check">
                ...
                <tr>
                    ...
                    <th class="uk-width-2-10"><?= Uni::t('app', 'Hudud name') ?></th>
                    <th class="uk-width-2-10 uk-text-center"><?= Uni::t('app', 'City') ?></th>
                ...
                <?php if (!empty($tuman->models)): ?>
                    <? foreach ($tuman->models as $model) { ?>
                        <tr id='row_<?=$model->id?>'>
                            ...
                            <td class="uk-width-2-10 uk-text-center"><?= ($model->tuman)?$model->tuman->name:Uni::t('app',"Not set")?></td>
...
<?= uni\widgets\LinkPager::widget([
    ...
    <div class="uk-modal" id="modal_add_direction">
        <div class="uk-modal-dialog">
            ...
            <div class="uk-form-row">
                <?//= $form->field($new, 'tuman_id')->dropDownList($map) ?>
            </div>
            <input type="hidden" name="Uchastka[tuman_id]" value="<?=$user->tuman_id?>"/>
            <input type="hidden" name="Uchastka[viloyat_id]" value="<?=$user->viloyat_id?>"/>
...
<!--                    <td id="modal_view_viloyat"></td>-->
    ...
    <div class="uk-modal" id="modal_edit_direction">
        <div class="uk-modal-dialog">
            ...
            <div class="uk-form-row">
                <?//= $form->field($new, 'tuman_id')->dropDownList($map, ['id' => 'dir-speciality']) ?>
            </div>
...
$this->registerJs(' /*ko`rish oynasi*/
    $("i.eye").click(function(e){
        ...
        $.get("../hudud/view",{id: model},function(response){
            if(response!="error"){
