...
class VaksinaController extends Controller
{
    ...
    public function actionViloyat($id)
    {
        ...
        return $this->render('viloyat', ['viloyat'=>$viloyat, "prixod"=>$prixod]);
    }

    public function actionTuman($item,$id)
    {
        $viloyat = new ActiveDataProvider([
            'query' => VkTuman::find()->where(['viloyat_id'=>$id, 'vil_prixod'=>$item])->orderBy(["id"=>SORT_DESC])
        ]);
        $viloyat->pagination->pageSize=20;
        $nomer = VkViloyat::findOne($item);
//        $prixod = Prixod::findOne($item);

        return $this->render('tuman', ['viloyat'=>$viloyat, "prixod"=>$nomer]);
    }

    public function actionUchastka($item,$id)
    {
        $viloyat = new ActiveDataProvider([
            'query' => VkUchastka::find()->where(['tuman_id'=>$id, 'tum_prixod'=>$item])->orderBy(["id"=>SORT_DESC])
        ]);
        $viloyat->pagination->pageSize=20;
        $nomer = VkTuman::findOne($item);
//        $prixod = Prixod::findOne($item);

        return $this->render('uchastka', ['viloyat'=>$viloyat, "prixod"=>$nomer]);
    }

    public function actionUser($item,$id)
    {
        $viloyat = new ActiveDataProvider([
            'query' => VaksinaPerform::find()->where(['uchastka_id'=>$id, 'vaksina_id'=>$item])->orderBy(["id"=>SORT_DESC])
        ]);
        $viloyat->pagination->pageSize=20;
        $nomer = VkUchastka::findOne(['vaksina_id'=>$item, 'uchastka_id'=>$id]);
//        $prixod = Prixod::findOne($item);

        return $this->render('user', ['viloyat'=>$viloyat, "prixod"=>$nomer]);
    }

    public function actionPerform($id)
    {
        $model = VaksinaPerform::findOne($id);
        return $this->render('perform', ['model'=>$model]);
    }

    public function actionAll()
    {
        $new = new VaksinaPerform();
        $viloyat =$new->search(Uni::$app->request->queryParams);
        $viloyat->pagination->pageSize=20;
        return $this->render('all', ['model'=>$viloyat]);
    }
