...
//            $model->updated_at = time();
    ...
    public function actionSaveviloyat()
    {
        ...
        if ($model->load($data)) {
            if(!$model->{'vaksina_id'}) $model->{'vaksina_id'} = $data['VkViloyat']['vaksina_id'];
            if(!$model->{'proxod_id'}) $model->{'proxod_id'} = $data['VkViloyat']['proxod_id'];
            $count = Prixod::find()->where(['status'=>Prixod::STATUS_ACTIVE, 'vaksina_id'=>$model->{'vaksina_id'}, 'id'=>$model->{'proxod_id'}])->one();
            //var_dump($data['VkViloyat']['proxod_id']);exit;
//            $jami =Prixod::find()->where(['id'=>$model->{'proxod_id'}])->one();
//            return ['status' => $model->{'proxod_id'}];
            if (empty($count)) return ['status'=>'error 111'];
            ...
            if ($count->count >= $count->ostatok+$model->{'vaksina_miqdor'}){
                ...
                if ($model->save()) {
                    ...
                    if($count->ostatok>=0){
                        ...
                        return ['status' => 'ostatok not enough'];
                    }

                } else {
                    return ['status'=>'hatolikda'];
                }
            }
            else return ['status'=>Uni::t('app', 'Not enough balance in inventory and inventory costs')];

        }
        return ['error'=>'ok','message'=>$model];
    }
...
//            return ['status' => $model->{'vil_prixod'}];
            ...
            if ($model->{'vaksina_miqdor'}==0) return ['status'=>Uni::t('app', '0 is not allowed')];
            if ($count->ostatok-$model->{'vaksina_miqdor'}>=0){
...
//            return ['status' => $model->{'uchastka_id'}];
            ...
            if ($count->ostatok-$model->{'vaksina_miqdor'} >= 0){
                ...
                $model->{'uchastka_id'} = $model->{'uchastka_id'}; /* User ID sini yozyapman*/
                $model->{'status'} = 0;
    ...
    public function actionList()
    {
        ...
        if (!empty($tuman)){
            foreach ($tuman as $item){
                if ($item->ostatok <= 0) continue;
                if(Uni::$app->language=='ru')echo "<option value='".$item->id."'>".$item->name_ru." ( ".$item->ostatok." ".$item->vaksina->unit->name." )</option>";
                else echo "<option value='".$item->id."'>".$item->name_uz." ( ".$item->ostatok." ".$item->vaksina->unit->name." )</option>";
            }
        }
    ...
    public function actionListcity()
    {
        ...
        $id = $_GET['id'];
        $prixod = VkViloyat::find()->where(['viloyat_id'=>$user->personal->viloyat_id,'status'=>VkViloyat::STATUS_ACTIVE, 'vaksina_id'=>$id])->all();
        if (!empty($prixod)){
            foreach ($prixod as $item){
                if ($item->ostatok<=0) continue;
                echo "<option value='".$item->id."'>".$item->nomer." ( ".$item->ostatok." ".$item->vaksina->unit->name." )</option>";
            }
        }
        else echo "<option>".$user->personal->viloyat_id."</option>";
    }
    ...
    public function actionListuser()
    {
        $id = $_GET['id'];
        $user = Uni::$app->getUser()->identity;
        $prixod = VkTuman::find()->where(['tuman_id'=>$user->personal->tuman_id, 'status'=>VkTuman::STATUS_ACTIVE, 'vaksina_id'=>$id])->all();
        // echo "<pre>";
        // var_dump($prixod);exit;
        if (!empty($prixod)){
            foreach ($prixod as $item){
                if ($item->ostatok<=0) continue;
                echo "<option value='".$item->id."'>".$item->nomer." ( ".$item->ostatok." ".$item->vaksina->unit->name." )</option>";
    ...
    public function actionViloyat($id)
    {
        ...
        if ($vil){
            ...
            $res = new ActiveDataProvider([
                'query' => VkTuman::find()->joinWith('tuman')->where("`vk_tuman`.`status`=1 and `vk_tuman`.`viloyat_id`=$id and `uni_tuman`.`status`=1")->orderBy(["name_$current->url"=>SORT_ASC])->distinct()
            ]);
...
//        $user_tuman = Uni::$app->getUser()->identity->viloyat_id;
        $tum = VkTuman::findOne(['tuman_id'=>$id]);
        if (!empty($tum)){
//            $user_tuman = $item; $userId = $id;
            ...
            $res = new ActiveDataProvider([
                'query' => UserModel::find()->where(['tuman_id'=>$id])->orderBy(["username"=>SORT_ASC])
            ]);
            $res->pagination->pageSize=20;
            ...
            if (Uni::$app->request->get('q')){
                $key = Uni::$app->request->get('q');
                $all = new ActiveDataProvider([
                    'query' => UserModel::find()->where(['tuman_id'=>$id])->andFilterWhere([
                        'or',
                        ['like', 'username', $key],
                        ['like', 'phone', $key],
                    ])
            ...
            $kutish = new ActiveDataProvider([
                'query' => UserModel::find()->joinWith('vkuchas')->where("`vk_uchastka`.`status`=0 and `vk_uchastka`.`tuman_id`=$id and `uni_user_security`.`status`=1")->orderBy(["username"=>SORT_ASC])->distinct()
            ]);
            $kutish->pagination->pageSize=20;

            $tuman = UserModel::find()->where(['tuman_id'=>$id])->orderBy(["username"=>SORT_ASC])->all();
            $map = ArrayHelper::map($tuman,'id','phone');
    //        else $map = ArrayHelper::map($tuman,'id','name_uz');

            $vil_vaksina = VkTuman::find()->where(['tuman_id'=>$id, 'status'=>VkTuman::STATUS_ACTIVE])->all();
    //        var_dump($vil_vaksina); die;
            $vaksina = [];
            foreach ($vil_vaksina as $item){
                $vaksina[] = Vaksina::find()->where(['status'=>Vaksina::STATUS_ACTIVE, 'id'=>$item->vaksina_id])->orderBy(["name_$current->url"=>SORT_ASC])->one();
            ...
            return $this->render('tuman',['data' => $res,'m'=>$m, 'tuman'=>$map, 'vaksina'=>$map2, 'kutish'=>$kutish, 'query'=>$all, 'tuman_id'=>$id,]);
        }
        else throw new ForbiddenHttpException('Sizda bu sahifa uchun ruxsat mavjud emas');
    }
