...
class HyturiController extends \app\components\Controller
{
    public $block="/left";
    public $cm="settings";
    ...
    public function actionIndex()
    {
        ...
        $data = new ActiveDataProvider([
            'query' => HayvonTuri::find()->where(['status'=>HayvonTuri::STATUS_ACTIVE])->orWhere(['status'=>HayvonTuri::STATUS_INACTIVE])->orderBy(["id"=>SORT_DESC])
        ]);
        $data->pagination->pageSize=20;
        if (Uni::$app->request->get('q')){
            $key = Uni::$app->request->get('q');
            $all = new ActiveDataProvider([
                'query' => HayvonTuri::find()->where("`vk_hayvon_turi`.`status`!=9 and (`vk_hayvon_turi`.`name_ru` like '%" . $key . "%' or `vk_hayvon_turi`.`name_uz` like '%" . $key . "%')")
            ]);
        }
        else $all = false;
...
//            $direction->updated_at = time();
    ...
    /**
     * Deletes an existing HayvonTuri model.
...
//            $model->updated_at = time();
    ...
    /**
     * Finds the HayvonTuri model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return HayvonTuri the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = HayvonTuri::findOne($id)) !== null) {
            return $model;
        } else {
