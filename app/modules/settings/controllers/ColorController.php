...
//use app\models\search\HayvonRangiSearch;
...
/**
 * ColorController implements the CRUD actions for HayvonRangi model.
 */
class ColorController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $data = new ActiveDataProvider([
            'query' => HayvonRangi::find()->where(['status'=>HayvonRangi::STATUS_ACTIVE])->orWhere(['status'=>HayvonRangi::STATUS_INACTIVE])->orderBy(["id"=>SORT_DESC])
        ]);
        $data->pagination->pageSize=20;
        if (Uni::$app->request->get('q')){
            $key = Uni::$app->request->get('q');
            $all = new ActiveDataProvider([
                'query' => HayvonRangi::find()->where("`vk_hayvon_rangi`.`status`!=9 and (`vk_hayvon_rangi`.`name_ru` like '%" . $key . "%' or `vk_hayvon_rangi`.`name_uz` like '%" . $key . "%')")
            ]);
        }
        else $all = false;
    ...
    /**
     * Displays a single HayvonRangi model.
     * @param integer $id
     * @return mixed
     */
...
//        $model->user_id = Uni::$app->getUser()->getId();
    ...
    /**
     * Creates a new HayvonRangi model.
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Uni::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->id_HayvonRangi]);
        } else {
    ...
    /**
     * Updates an existing HayvonRangi model.
...
//            $direction->updated_at = time();
    ...
    /**
     * Deletes an existing HayvonRangi model.
...
//            $model->updated_at = time();
    ...
    /**
     * Finds the HayvonRangi model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return HayvonRangi the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = HayvonRangi::findOne($id)) !== null) {
            return $model;
        } else {
