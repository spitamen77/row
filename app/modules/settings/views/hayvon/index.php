...
$this->registerJs('Muxr.saveHayvon();Muxr.openClearedDirection();Muxr.openDirectionForm();Muxr.openEditHayvonForm();Muxr.openDeleteDirectionForm();Muxr.editHayvonStatus();');
$q=false;
if(isset($_GET['q'])){
    $q=$_GET['q'];
}
...
if ($current->url=="ru") $rang = ArrayHelper::map($color,'id','name_ru');
else $rang = ArrayHelper::map($color,'id','name_uz');
$this->title = Uni::t('app', 'Animals');
?>
    <div class="block-process" style="margin-bottom:10px;">
        <div class="uk-grid">
            <div class="uk-width-1-2">
                <a class="md-btn md-btn-success md-btn-small" href="<?=Url::to('settings/hayvon/index')?>"><?=Uni::t('app','All')?></a>
                <button id="modal_add_direction_btn" data-uk-modal="{bgClose:true;modal:true}" class="md-btn md-btn-facebook md-btn-small" ><?=Uni::t('app', 'Add animal')?></button>
                <div class="uk-button-dropdown" data-uk-dropdown="{mode:'click'}">
                    <button class="md-btn md-btn-block md-btn-small"><?=Uni::t('app', 'Animal types')?> <i class="material-icons"></i></button>
                    <div class="uk-dropdown">
                        <ul class="uk-nav uk-nav-dropdown">
                            <li><a href="<?=Url::to('settings/hayvon/index')?>" class="viloyat"><?= Uni::t('app',"All") ?></a></li>
                            <?php foreach ($items as $item): ?>
                                <li><a href="<?=Url::to('settings/hayvon/index?slug=item&drug='.$item->id)?>" class="viloyat" data-id="<?=$item->id?>"><?= ($item->name)?$item->name:Uni::t('app',"Not set") ?></a></li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
                </div>
        ...
        <div class="md-card-content">
            <table class="uk-table uk-table-nowrap table_check">
                ...
                <tr>
                    ...
                    <th class="uk-width-2-10"><?= Uni::t('app', 'Animal name') ?></th>
                    <th class="uk-width-2-10 uk-text-center"><?= Uni::t('app', 'Animal type') ?></th>
                    <th class="uk-width-2-10 uk-text-center"><?= Uni::t('app', 'Color') ?></th>
                ...
                <tbody>
                <?php if (!empty($query)) $hayvon->models = $query->models; ?>
                <?php  if (!empty($hayvon->models)): ?>
                    <? foreach ($hayvon->models as $model) { ?>
                        <tr id='row_<?=$model->id?>'>
                            <? $st = $model->status; ?>
                            <td class="uk-text-center uk-table-middle small_col">
                                <input type="checkbox" data-md-icheck class="check_row"/>
                            </td>
                            <td class="uk-width-2-10"><?= ($model->name)?$model->name:Uni::t('app',"Not set") ?></td>
                            <td class="uk-width-2-10 uk-text-center"><?= ($model->turi)?$model->turi->name:Uni::t('app',"Not set")?></td>
                            <td class="uk-width-2-10 uk-text-center"><?=($model->color)?$model->color->name:Uni::t('app',"Not set")?></td>
                            ...
                            <td class="uk-text-center">
                                <a class="modal-edit-direction" data-id="<?=$model->id?>" data-title="<?=$model->name_ru?>" data-short="<?=$model->name_uz?>" data-speciality="<?=$model->hayvon_turi_id?>">
...
<?= uni\widgets\LinkPager::widget([
    ...
    <div class="uk-modal" id="modal_add_direction">
        <div class="uk-modal-dialog">
            ...
            <div class="uk-form-row">
                <?= $form->field($new, 'hayvon_turi_id')->dropDownList($map) ?>
            </div>
            <div class="uk-form-row">
                <?= $form->field($new, 'rang_id')->dropDownList($rang) ?>
            </div>
    ...
    <div class="uk-modal" id="modal_view" aria-hidden="true" style="display: none; overflow-y: scroll;">
        <div class="uk-modal-dialog" style="top: 34.5px;">
            ...
            <table class="uk-table uk-table-hover uk-table-bordered">
                <thead>
                <tr>
                    <th><?=Uni::t('app', 'Animal type')?></th>
                    <th colspan="2" style="text-align: center"><?=Uni::t('app', 'Animal name')?></th>
                    <th><?=Uni::t('app', 'Animal color')?></th>
                </tr>
                </thead>
                <tbody>
                <tr>
    ...
    <div class="uk-modal" id="modal_edit_direction">
        <div class="uk-modal-dialog">
            ...
            <div class="uk-form-row">
                <?= $form->field($new, 'hayvon_turi_id')->dropDownList($map, ['id' => 'dir-speciality']) ?>
            </div>
            <div class="uk-form-row">
                <?= $form->field($new, 'rang_id')->dropDownList($rang, ['id' => 'dir-speciality']) ?>
            </div>
...
$this->registerJs(' /*ko`rish oynasi*/
    $("i.eye").click(function(e){
        ...
        $.get("../hayvon/view",{id: model},function(response){
            if(response!="error"){
                $("#modal_view_text").html(response.name_ru);
                $("#modal_view_text2").html(response.name_uz);
                $("#modal_view_viloyat").html(response.hayvon_turi_id);
                $("#modal_view_color").html(response.color);
            }
            else {
                $("#modal_view_text").html(response);
                $("#modal_view_viloyat").html(response);
            }
        });
    });
