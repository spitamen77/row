...
$this->registerJs('Muxr.saveKasallik();Muxr.openClearedDirection();Muxr.openDirectionForm();Muxr.openEditVaksinaForm();Muxr.openDeleteDirectionForm();Muxr.editKasallikStatus();');
$q=false;
if(isset($_GET['q'])){
    $q=$_GET['q'];
}
...
$this->title = Uni::t('app', 'Disease');
?>
    <div class="block-process" style="margin-bottom:10px;">
        <div class="uk-grid">
            <div class="uk-width-1-2">
                <a class="md-btn md-btn-success md-btn-small" href="<?=Url::to('settings/kasallik/index')?>"><?=Uni::t('app','All')?></a>
                <button id="modal_add_direction_btn" data-uk-modal="{bgClose:true;modal:true}" class="md-btn md-btn-facebook md-btn-small" ><?=Uni::t('app', 'Add disease')?></button>
                <div class="uk-button-dropdown" data-uk-dropdown="{mode:'click'}">
                    <button class="md-btn md-btn-block md-btn-small"><?=Uni::t('app', 'Disease types')?> <i class="material-icons"></i></button>
                    <div class="uk-dropdown">
                        <ul class="uk-nav uk-nav-dropdown">
                            <li><a href="<?=Url::to('settings/kasallik/index')?>" class="viloyat"><?= Uni::t('app',"All") ?></a></li>
                            <?php foreach ($items as $item): ?>
                                <li><a href="<?=Url::to('settings/kasallik/index?slug=item&drug='.$item->id)?>" class="viloyat" data-id="<?=$item->id?>"><?= ($item->name)?$item->name:Uni::t('app',"Not set") ?></a></li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
                </div>
        ...
        <div class="md-card-content">
            <table class="uk-table uk-table-nowrap table_check">
                ...
                <tr>
                    ...
                    <th class="uk-width-2-10 uk-text-center"><?= Uni::t('app', 'Disease type') ?></th>
                    ...
                    <? foreach ($vaksina->models as $model) { ?>
                        <tr id='row_<?=$model->id?>'>
                            ...
                            <td class="uk-text-center">
                                <a class="modal-edit-direction" data-id="<?=$model->id?>" data-title="<?=$model->name_ru?>" data-short="<?=$model->name_uz?>" data-speciality="<?=$model->kasal_id?>">
...
<?= uni\widgets\LinkPager::widget([
    ...
    <div class="uk-modal" id="modal_add_direction">
        <div class="uk-modal-dialog">
            ...
            <div class="uk-form-row">
                <?= $form->field($new, 'kasal_id')->dropDownList($map) ?>
            </div>
    ...
    <div class="uk-modal" id="modal_view" aria-hidden="true" style="display: none; overflow-y: scroll;">
        <div class="uk-modal-dialog" style="top: 34.5px;">
            <div class="uk-modal-header">
                <h3 class="uk-modal-title"><?= Uni::t('app', 'Disease') ?></h3>
            </div>
            <table class="uk-table uk-table-hover uk-table-bordered">
                <thead>
                <tr>
                    <th><?=Uni::t('app', 'Disease type')?></th>
                    <th  colspan="2" style="text-align: center"><?=Uni::t('app', 'Disease name')?></th>
                </tr>
                </thead>
                <tbody>
                <tr>
    ...
    <div class="uk-modal" id="modal_edit_direction">
        <div class="uk-modal-dialog">
            ...
            <div class="uk-form-row">
                <?= $form->field($new, 'kasal_id')->dropDownList($map, ['id' => 'dir-speciality']) ?>
            </div>
...
$this->registerJs(' /*ko`rish oynasi*/
    $("i.eye").click(function(e){
        ...
        $.get("../kasallik/view",{id: model},function(response){
            if(response!="error"){
                $("#modal_view_text").html(response.name_ru);
                $("#modal_view_text2").html(response.name_uz);
                $("#modal_view_viloyat").html(response.kasal_id);
            }
            else {
