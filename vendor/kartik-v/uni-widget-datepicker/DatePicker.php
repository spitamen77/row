...
/**
 * DatePicker widget is a Uni2 wrapper for the Bootstrap DatePicker plugin. The
...
class DatePicker extends InputWidget
{
    ...
    protected function validateConfig()
    {
        ...
        if ($this->type === self::TYPE_RANGE) {
            ...
            if (!class_exists('\\kartik\\field\\FieldRangeAsset')) {
                self::err(
                    "The uni-field-range extension is not installed and is a pre-requisite for a DatePicker " .
                    "RANGE type. To install this extension run this command on your console: \n\n" .
                    "php composer.phar require kartik-v/uni-field-range \"*\""
                );
            }
        }
        if (!isset($this->form)) {
        ...
        if (!$this->form instanceof ActiveForm) {
            self::err("The 'form' property must be of type \\uni\\widgets\\ActiveForm");
        }
        if (!$this->hasModel()) {
    ...
    protected function renderAddon(&$options, $type = 'picker')
    {
        ...
        if ($title !== false && empty($title)) {
            $options['title'] = ($type === 'picker') ? Uni::t('kvdate', 'Select date') :
                Uni::t('kvdate', 'Clear field');
        }
