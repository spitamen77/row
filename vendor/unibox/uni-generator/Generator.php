...
abstract class Generator extends Model
{
    ...
    public $template;
    /**
     * @var boolean whether the strings will be generated using `Uni::t()` or normal strings.
     */
    public $enableI18N = false;
    /**
     * @var string the message category used by `Uni::t()` when `$enableI18N` is `true`.
     * Defaults to `app`.
     */
     ...
     * Please refer to [[\uni\generator\generators\controller\Generator::generate()]] as an example
    ...
    public function init()
    {
        ...
        foreach ($this->templates as $i => $template) {
            $this->templates[$i] = Uni::getAlias($template);
        }
    }
    ...
    {
        return [
            'enableI18N' => 'This indicates whether the generator should generate strings using <code>Uni::t()</code> method.
                Set this to <code>true</code> if you are planning to make your application translatable.',
            'messageCategory' => 'This is the category used by <code>Uni::t()</code> in case you enable I18N.',
        ];
    }
    ...
    {
        return Uni::$app->getRuntimePath() . '/generator-' . Uni::getVersion() . '/' . str_replace('\\', '-', get_class($this)) . '.json';
    }
    ...
    public function validateNewClass($attribute, $params)
    {
        ...
        if (($pos = strrpos($class, '\\')) === false) {
            ...
            $path = Uni::getAlias('@' . str_replace('\\', '/', $ns), false);
            if ($path === false) {
     ...
     * @param array  $placeholders the placeholders to use by `Uni::t()`
     */
    public function generateString($string = '', $placeholders = [])
    {
        ...
        if ($this->enableI18N) {
            ...
            $str = "Uni::t('" . $this->messageCategory . "', '" . $string . "'" . $ph . ")";
        } else {
