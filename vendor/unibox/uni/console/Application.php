 ...
 *
 * Application extends from [[\uni\base\Application]] by providing functionalities that are
 * specific to console requests. In particular, it deals with console requests
 * through a command-based approach:
 *
 * - A console application consists of one or several possible user commands;
 * - Each user command is implemented as a class extending [[\uni\console\Controller]];
 ...
 *
 * ```
 * uni <route> [--param1=value1 --param2 ...]
 * ```
 *
...
class Application extends \uni\base\Application
{
        ...
        if (!empty($_SERVER['argv'])) {
            ...
            foreach ($_SERVER['argv'] as $param) {
                if (strpos($param, $option) !== false) {
                    ...
                    if (!empty($path) && is_file($file = Uni::getAlias($path))) {
                        return require($file);
                    } else {
    ...
    public function init()
    {
        ...
        if (!isset($this->controllerMap['help'])) {
            $this->controllerMap['help'] = 'uni\console\controllers\HelpController';
        }
    }
    ...
    {
        return [
            'message' => 'uni\console\controllers\MessageController',
            'help' => 'uni\console\controllers\HelpController',
            'migrate' => 'uni\console\controllers\MigrateController',
            'cache' => 'uni\console\controllers\CacheController',
            'asset' => 'uni\console\controllers\AssetController',
            'fixture' => 'uni\console\controllers\FixtureController',
            'serve' => 'uni\console\controllers\ServeController',
        ];
    }
    ...
    public function coreComponents()
    {
        return array_merge(parent::coreComponents(), [
            'request' => ['class' => 'uni\console\Request'],
            'response' => ['class' => 'uni\console\Response'],
            'errorHandler' => ['class' => 'uni\console\ErrorHandler'],
        ]);
    }
}
