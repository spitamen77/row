 ...
 *
 *     uni cache/flush first second third
 *
...
class CacheController extends Controller
{
    ...
    public function actionFlush()
    {
        ...
        foreach ($caches as $name => $class) {
            $cachesInfo[] = [
                'name' => $name,
                'class' => $class,
                'is_flushed' =>  Uni::$app->get($name)->flush(),
            ];
        }
    ...
    {
        $connection = Uni::$app->get($db, false);
        if ($connection === null) {
            $this->stdout("Unknown component \"$db\".\n", Console::FG_RED);
            return self::EXIT_CODE_ERROR;
        }

        if (!$connection instanceof \uni\db\Connection) {
            $this->stdout("\"$db\" component doesn't inherit \\uni\\db\\Connection.\n", Console::FG_RED);
            return self::EXIT_CODE_ERROR;
    ...
    {
        $caches = [];
        $components = Uni::$app->getComponents();
        $findAll = ($cachesNames === []);
