...
abstract class BaseMigrateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        if (parent::beforeAction($action)) {
            $path = Uni::getAlias($this->migrationPath);
            if (!is_dir($path)) {
                if ($action->id !== 'create') {
            ...
            $version = Uni::getVersion();
            $this->stdout("Uni Migration Tool (based on Uni v{$version})\n\n");
     ...
     *
     * ```
     * uni migrate     # apply all new migrations
     * uni migrate 3   # apply the first 3 new migrations
     * ```
     *
     ...
     *
     * ```
     * uni migrate/down     # revert the last migration
     * uni migrate/down 3   # revert the last 3 migrations
     * uni migrate/down all # revert all migrations
     * ```
     *
     ...
     *
     * ```
     * uni migrate/redo     # redo the last applied migration
     * uni migrate/redo 3   # redo the last 3 applied migrations
     * uni migrate/redo all # redo all migrations
     * ```
     *
     ...
     *
     * ```
     * uni migrate/to 101129_185401                      # using timestamp
     * uni migrate/to m101129_185401_create_user_table   # using full name
     * uni migrate/to 1392853618                         # using UNIX timestamp
     * uni migrate/to "2014-02-15 13:00:50"              # using strtotime() parseable string
     * ```
     *
     ...
     *
     * ```
     * uni migrate/mark 101129_185401                      # using timestamp
     * uni migrate/mark m101129_185401_create_user_table   # using full name
     * ```
     *
     ...
     *
     * ```
     * uni migrate/history     # showing the last 10 migrations
     * uni migrate/history 5   # showing the last 5 migrations
     * uni migrate/history all # showing the whole history
     * ```
     *
     * @param integer $limit the maximum number of migrations to be displayed.
     * If it is "all", the whole migration history will be displayed.
     * @throws \uni\console\Exception if invalid limit value passed
     */
     ...
     *
     * ```
     * uni migrate/new     # showing the first 10 new migrations
     * uni migrate/new 5   # showing the first 5 new migrations
     * uni migrate/new all # showing all new migrations
     * ```
     *
     ...
     *
     * ```
     * uni migrate/create create_user_table
     * ```
     *
    ...
    public function actionCreate($name)
    {
        ...
        if ($this->confirm("Create new migration '$file'?")) {
            if (preg_match('/^create_junction_(.+)_and_(.+)$/', $name, $matches)) {
                $firstTable = mb_strtolower($matches[1], Uni::$app->charset);
                $secondTable = mb_strtolower($matches[2], Uni::$app->charset);

                $content = $this->renderFile(Uni::getAlias($this->generatorTemplateFiles['create_junction']), [
                    'className' => $className,
                ...
                $content = $this->renderFile(Uni::getAlias($this->generatorTemplateFiles['add_column']), [
                    'className' => $className,
                    'table' => mb_strtolower($matches[2], Uni::$app->charset),
                    'fields' => $this->fields
                ]);
            } elseif (preg_match('/^drop_(.+)_from_(.+)$/', $name, $matches)) {
                $content = $this->renderFile(Uni::getAlias($this->generatorTemplateFiles['drop_column']), [
                    'className' => $className,
                    'table' => mb_strtolower($matches[2], Uni::$app->charset),
                    'fields' => $this->fields
                ]);
                ...
                $content = $this->renderFile(Uni::getAlias($this->generatorTemplateFiles['create_table']), [
                    'className' => $className,
                    'table' => mb_strtolower($matches[1], Uni::$app->charset),
                    'fields' => $this->fields
                ]);
            } elseif (preg_match('/^drop_(.+)$/', $name, $matches)) {
                $this->addDefaultPrimaryKey();
                $content = $this->renderFile(Uni::getAlias($this->generatorTemplateFiles['drop_table']), [
                    'className' => $className,
                    'table' => mb_strtolower($matches[1], Uni::$app->charset),
                    'fields' => $this->fields
                ]);
            } else {
                $content = $this->renderFile(Uni::getAlias($this->templateFile), ['className' => $className]);
            }
     ...
     * @param string $class the migration class name
     * @return \uni\db\MigrationInterface the migration instance
     */
