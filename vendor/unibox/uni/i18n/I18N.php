 ...
 *
 * I18N is configured as an application component in [[\uni\base\Application]] by default.
 * You can access that instance via `Uni::$app->i18n`.
 *
...
class I18N extends Component
{
     ...
     *
     * The category "uni" and "app" are always defined. The former refers to the messages used in the Uni core
     ...
     * stored under "@uni/messages" and "@app/messages", respectively.
     *
    ...
    {
        parent::init();
        if (!isset($this->translations['uni']) && !isset($this->translations['uni*'])) {
            $this->translations['uni'] = [
                'class' => 'uni\i18n\PhpMessageSource',
                'sourceLanguage' => 'en-US',
                'basePath' => '@uni/messages',
            ];
        }
        if (!isset($this->translations['app']) && !isset($this->translations['app*'])) {
            $this->translations['app'] = [
                'class' => 'uni\i18n\PhpMessageSource',
                'sourceLanguage' => Uni::$app->sourceLanguage,
    ...
    public function format($message, $params, $language)
    {
        ...
        if (preg_match('~{\s*[\d\w]+\s*,~u', $message)) {
            ...
            if ($result === false) {
                ...
                Uni::warning("Formatting message for language '$language' failed with error: $errorMessage. The message being formatted was: $message.", __METHOD__);
    ...
    public function getMessageFormatter()
    {
        if ($this->_messageFormatter === null) {
            ...
            $this->_messageFormatter = Uni::createObject($this->_messageFormatter);
        }
     ...
     * Can be given as array or string configuration that will be given to [[Uni::createObject]] to create an instance
    ...
    public function getMessageSource($category)
    {
        if (isset($this->translations[$category])) {
            ...
            if ($source instanceof MessageSource) {
                ...
                return $this->translations[$category] = Uni::createObject($source);
            }
        } else {
            ...
            foreach ($this->translations as $pattern => $source) {
                if (strpos($pattern, '*') > 0 && strpos($category, rtrim($pattern, '*')) === 0) {
                    if ($source instanceof MessageSource) {
                        ...
                        return $this->translations[$category] = $this->translations[$pattern] = Uni::createObject($source);
                    }
                }
            }
            // match '*' in the last
            if (isset($this->translations['*'])) {
                ...
                if ($source instanceof MessageSource) {
                    ...
                    return $this->translations[$category] = $this->translations['*'] = Uni::createObject($source);
                }
            }
        }
