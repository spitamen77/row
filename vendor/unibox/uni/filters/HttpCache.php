...
class HttpCache extends ActionFilter
{
    ...
    public function beforeAction($action)
    {
        ...
        $verb = Uni::$app->getRequest()->getMethod();
    ...
    protected function validateCache($lastModified, $etag)
    {
        if (isset($_SERVER['HTTP_IF_NONE_MATCH'])) {
            ...
            return $etag !== null && in_array($etag, Uni::$app->request->getEtags(), true);
    ...
    protected function sendCacheControlHeader()
    {
        if ($this->sessionCacheLimiter !== null) {
            if ($this->sessionCacheLimiter === '' && !headers_sent() && Uni::$app->getSession()->getIsActive()) {
        ...
        $headers = Uni::$app->getResponse()->getHeaders();
        $headers->set('Pragma');
