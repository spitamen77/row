...
use uni\web\TooManyRequestsHttpException;
 ...
 *     return [
 *         'rateLimiter' => [
 *             'class' => \uni\filters\RateLimiter::className(),
 *         ],
 *     ];
 * }
 * ```
...
class RateLimiter extends ActionFilter
{
     ...
     * If not set, it will take the value of `Uni::$app->user->getIdentity(false)`.
     */
    public $user;
    /**
    ...
    {
        $user = $this->user ? : (Uni::$app->getUser() ? Uni::$app->getUser()->getIdentity(false) : null);
        if ($user instanceof RateLimitInterface) {
            Uni::trace('Check rate limit', __METHOD__);
            $this->checkRateLimit(
                $user,
                $this->request ? : Uni::$app->getRequest(),
                $this->response ? : Uni::$app->getResponse(),
                $action
            );
        } elseif ($user) {
            Uni::info('Rate limit skipped: "user" does not implement RateLimitInterface.', __METHOD__);
        } else {
            Uni::info('Rate limit skipped: user not logged in.', __METHOD__);
        }
        return true;
    }
