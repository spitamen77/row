...
class NumberValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = $this->integerOnly ? Uni::t('uni', '{attribute} must be an integer.')
                : Uni::t('uni', '{attribute} must be a number.');
        }
    ...
    {
        if (is_array($value)) {
            return [Uni::t('uni', '{attribute} is invalid.'), []];
        }
        $pattern = $this->integerOnly ? $this->integerPattern : $this->numberPattern;
        if (!preg_match($pattern, "$value")) {
            return [$this->message, []];
    ...
    public function clientValidateAttribute($model, $attribute, $view)
    {
        ...
        if ($this->min !== null) {
            ...
            // https://github.com/unisoft/uni2/issues/3118
        ...
        return 'uni.validation.number(value, messages, ' . Json::htmlEncode($options) . ');';
    }
}
