...
class StringValidator extends Validator
{
     ...
     * If this property is not set, [[\uni\base\Application::charset]] will be used.
     */
    public $encoding;
    ...
    public function init()
    {
        ...
        if ($this->encoding === null) {
            $this->encoding = Uni::$app->charset;
        }
        if ($this->message === null) {
            $this->message = Uni::t('uni', '{attribute} must be a string.');
        }
        if ($this->min !== null && $this->tooShort === null) {
            $this->tooShort = Uni::t('uni', '{attribute} should contain at least {min, number} {min, plural, one{character} other{characters}}.');
        }
        if ($this->max !== null && $this->tooLong === null) {
            $this->tooLong = Uni::t('uni', '{attribute} should contain at most {max, number} {max, plural, one{character} other{characters}}.');
        }
        if ($this->length !== null && $this->notEqual === null) {
            $this->notEqual = Uni::t('uni', '{attribute} should contain {length, number} {length, plural, one{character} other{characters}}.');
        }
    }
    ...
    public function clientValidateAttribute($model, $attribute, $view)
    {
        ...
        if ($this->min !== null) {
            ...
            $options['tooShort'] = Uni::$app->getI18n()->format($this->tooShort, [
                'attribute' => $label,
                'min' => $this->min,
            ], Uni::$app->language);
        }
        if ($this->max !== null) {
            ...
            $options['tooLong'] = Uni::$app->getI18n()->format($this->tooLong, [
                'attribute' => $label,
                'max' => $this->max,
            ], Uni::$app->language);
        }
        if ($this->length !== null) {
            ...
            $options['notEqual'] = Uni::$app->getI18n()->format($this->notEqual, [
                'attribute' => $label,
                'length' => $this->length,
            ], Uni::$app->language);
        }
        ...
        return 'uni.validation.string(value, messages, ' . json_encode($options, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . ');';
    }
}
