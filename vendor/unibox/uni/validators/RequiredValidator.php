...
class RequiredValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = $this->requiredValue === null ? Uni::t('uni', '{attribute} cannot be blank.')
                : Uni::t('uni', '{attribute} must be "{requiredValue}".');
        }
    }
    ...
    public function clientValidateAttribute($model, $attribute, $view)
    {
        ...
        $options['message'] = Uni::$app->getI18n()->format($options['message'], [
            'attribute' => $model->getAttributeLabel($attribute),
        ], Uni::$app->language);

        ValidationAsset::register($view);

        return 'uni.validation.required(value, messages, ' . json_encode($options, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . ');';
    }
}
