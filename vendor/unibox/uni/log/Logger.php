 ...
 *
 * A Logger instance can be accessed via `Uni::getLogger()`. You can call the method [[log()]] to record a single log message.
...
class Logger extends Component
{
    ...
    public function log($message, $level, $category = 'application')
    {
        ...
        if ($this->traceLevel > 0) {
            ...
            foreach ($ts as $trace) {
                if (isset($trace['file'], $trace['line']) && strpos($trace['file'], UNI2_PATH) !== 0) {
    ...
    public function flush($final = false)
    {
        ...
        // https://github.com/unisoft/uni2/issues/5619
     ...
     * defined by constant `UNI_BEGIN_TIME` which is evaluated at the beginning
     * of [[\uni\BaseUni]] class file.
     * @return float the total elapsed time in seconds for current request.
     */
    public function getElapsedTime()
    {
        return microtime(true) - UNI_BEGIN_TIME;
    }
     ...
     * For example, 'uni\db\*' will match categories starting with 'uni\db\',
     * such as 'uni\db\Connection'.
    ...
    {
        $timings = $this->getProfiling(['uni\db\Command::query', 'uni\db\Command::execute']);
        $count = count($timings);
        $time = 0;
