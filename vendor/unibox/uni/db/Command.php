 ...
 *
 * @see http://www.uniframework.com/doc-2.0/guide-db-dao.html
 *
...
class Command extends Component
{
    ...
    /**
     * @var \uni\caching\Dependency the dependency to be associated with the cached query result for this command
     * @see cache()
     */
     ...
     * @param \uni\caching\Dependency $dependency the cache dependency associated with the cached query result.
     ...
     * @param string $type the column type. [[\uni\db\QueryBuilder::getColumnType()]] will be called
     ...
     * @since alfa version.1 this method is protected (was private before).
     */
    protected function queryInternal($method, $fetchMode = null)
    {
        $rawSql = $this->getRawSql();

        Uni::info($rawSql, 'uni\db\Command::query');

        if ($method !== '') {
            $info = $this->db->getQueryCacheInfo($this->queryCacheDuration, $this->queryCacheDependency);
            if (is_array($info)) {
                /* @var $cache \uni\caching\Cache */
                $cache = $info[0];
                $cacheKey = [
                    __CLASS__,
                    $method,
                ...
                if (is_array($result) && isset($result[0])) {
                    Uni::trace('Query result served from cache', 'uni\db\Command::query');
                    return $result[0];
                }
            }
        }
        ...
        $token = $rawSql;
        try {
            Uni::beginProfile($token, 'uni\db\Command::query');
            ...
            Uni::endProfile($token, 'uni\db\Command::query');
        } catch (\Exception $e) {
            Uni::endProfile($token, 'uni\db\Command::query');
            throw $this->db->getSchema()->convertException($e, $rawSql);
        }
        ...
        if (isset($cache, $cacheKey, $info)) {
            ...
            Uni::trace('Saved query result in cache', 'uni\db\Command::query');
        }
