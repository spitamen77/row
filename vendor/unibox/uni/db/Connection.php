 ...
 *
 * ```php
 * $connection = new \uni\db\Connection([
 *     'dsn' => $dsn,
 *     'username' => $username,
 *     'password' => $password,
 * ]);
...
class Connection extends Component
{
     ...
     * schema class name or configuration. Please refer to [[Uni::createObject()]] for
     * details on how to specify a configuration.
     *
     * This property is mainly used by [[getSchema()]] when fetching the database schema information.
     * You normally do not need to set this property unless you want to use your own
     * [[Schema]] class to support DBMS that is not supported by Uni.
     */
    public $schemaMap = [
        'pgsql' => 'uni\db\pgsql\Schema', // PostgreSQL
        'mysqli' => 'uni\db\mysql\Schema', // MySQL
        'mysql' => 'uni\db\mysql\Schema', // MySQL
        'sqlite' => 'uni\db\sqlite\Schema', // sqlite 3
        'sqlite2' => 'uni\db\sqlite\Schema', // sqlite 2
        'sqlsrv' => 'uni\db\mssql\Schema', // newer MSSQL driver on MS Windows hosts
        'oci' => 'uni\db\oci\Schema', // Oracle driver
        'mssql' => 'uni\db\mssql\Schema', // older MSSQL driver on MS Windows hosts
        'dblib' => 'uni\db\mssql\Schema', // dblib drivers on GNU/Linux (and maybe other OSes) hosts
        'cubrid' => 'uni\db\cubrid\Schema', // CUBRID
    ];
    /**
     * @var string Custom PDO wrapper class. If not set, it will use "PDO" or "uni\db\mssql\PDO" when MSSQL is used.
     */
    public $pdoClass;
    /**
     ...
     * Use 0 to indicate that the cached data will never expire.
     * @param \uni\caching\Dependency $dependency the cache dependency associated with the cached query results.
     ...
     * @param \uni\caching\Dependency $dependency the preferred caching dependency. If null, it will be ignored.
    ...
    public function getQueryCacheInfo($duration, $dependency)
    {
        ...
        if ($duration === 0 || $duration > 0) {
            if (is_string($this->queryCache) && Uni::$app) {
                $cache = Uni::$app->get($this->queryCache, false);
            } else {
    ...
    {
        if ($this->pdo !== null) {
            Uni::trace('Closing DB connection: ' . $this->dsn, __METHOD__);
            $this->pdo = null;
            $this->_schema = null;
    ...
    protected function createPdoInstance()
    {
        ...
        if ($pdoClass === null) {
            ...
            if (isset($driver)) {
                if ($driver === 'mssql' || $driver === 'dblib') {
                    $pdoClass = 'uni\db\mssql\PDO';
                } elseif ($driver === 'sqlsrv') {
                    $pdoClass = 'uni\db\mssql\SqlsrvPDO';
                }
            }
        }
        ...
        if (strncmp('sqlite:@', $dsn, 8) === 0) {
            $dsn = 'sqlite:' . Uni::getAlias(substr($dsn, 7));
        }
    ...
    public function getSchema()
    {
        if ($this->_schema !== null) {
            ...
            if (isset($this->schemaMap[$driver])) {
                ...
                return $this->_schema = Uni::createObject($config);
            } else {
    ...
    protected function openFromPool(array $pool, array $sharedConfig)
    {
        ...
        $cache = is_string($this->serverStatusCache) ? Uni::$app->get($this->serverStatusCache, false) : $this->serverStatusCache;
        ...
        foreach ($pool as $config) {
            ...
            /* @var $db Connection */
            $db = Uni::createObject($config);

            try {
                $db->open();
                return $db;
            } catch (\Exception $e) {
                Uni::warning("Connection ({$config['dsn']}) failed: " . $e->getMessage(), __METHOD__);
                if ($cache instanceof Cache) {
