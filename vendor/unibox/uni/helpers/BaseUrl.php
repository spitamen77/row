...
class BaseUrl
{
     ...
     *
     * This method will use [[\uni\web\UrlManager]] to create a URL.
     *
     ...
     *
     * - If the route is an empty string, the current [[\uni\web\Controller::route|route]] will be used;
     * - If the route contains no slashes at all (e.g. `index`), it is considered to be an action ID
     *   of the current controller and will be prepended with [[\uni\web\Controller::uniqueId]];
     * - If the route has no leading slash (e.g. `site/index`), it is considered to be a route relative
     *   to the current module and will be prepended with the module's [[\uni\base\Module::uniqueId|uniqueId]].
     *
     ...
     * - `true`: returning an absolute base URL whose scheme is the same as that in [[\uni\web\UrlManager::hostInfo]].
    ...
    public static function toRoute($route, $scheme = false)
    {
        ...
        if ($scheme) {
            return Uni::$app->getUrlManager()->createAbsoluteUrl($route, is_string($scheme) ? $scheme : null);
        } else {
            return Uni::$app->getUrlManager()->createUrl($route);
        }
    }
    ...
    {
        $route = Uni::getAlias((string) $route);
        ...
        // relative route
        if (Uni::$app->controller === null) {
            throw new InvalidParamException("Unable to resolve the relative route: $route. No active controller is available.");
        }

        if (strpos($route, '/') === false) {
            // empty or an action ID
            return $route === '' ? Uni::$app->controller->getRoute() : Uni::$app->controller->getUniqueId() . '/' . $route;
        } else {
            // relative to module
            return ltrim(Uni::$app->controller->module->getUniqueId() . '/' . $route, '/');
        }
    }
     ...
     * [[\uni\web\UrlManager::hostInfo]]) will be returned. If `$url` is already an absolute URL, its scheme
    ...
    public static function to($url = '', $scheme = false)
    {
        ...
        $url = Uni::getAlias($url);
        if ($url === '') {
            $url = Uni::$app->getRequest()->getUrl();
        }
        ...
        if (($pos = strpos($url, ':')) === false || !ctype_alpha(substr($url, 0, $pos))) {
            ...
            $url = Uni::$app->getUrlManager()->getHostInfo() . '/' . ltrim($url, '/');
        }
    ...
    {
        $url = Uni::$app->getUrlManager()->getBaseUrl();
        if ($scheme) {
            $url = Uni::$app->getUrlManager()->getHostInfo() . $url;
            if (is_string($scheme) && ($pos = strpos($url, '://')) !== false) {
                $url = $scheme . substr($url, $pos);
            }
        }
     ...
     * later by [[previous()]]. If not set, it will use [[\uni\web\User::returnUrlParam]].
     * @see previous()
     */
    public static function remember($url = '', $name = null)
    {
        ...
        if ($name === null) {
            Uni::$app->getUser()->setReturnUrl($url);
        } else {
            Uni::$app->getSession()->set($name, $url);
        }
    }
     ...
     * If not set, it will use [[\uni\web\User::returnUrlParam]].
    ...
    {
        if ($name === null) {
            return Uni::$app->getUser()->getReturnUrl();
        } else {
            return Uni::$app->getSession()->get($name);
        }
    }

    /**
     * Returns the canonical URL of the currently requested page.
     * The canonical URL is constructed using the current controller's [[\uni\web\Controller::route]] and
     * [[\uni\web\Controller::actionParams]]. You may use the following code in the layout view to add a link tag
     * about canonical URL:
     *
     * ```php
    ...
    {
        $params = Uni::$app->controller->actionParams;
        $params[0] = Uni::$app->controller->getRoute();

        return Uni::$app->getUrlManager()->createAbsoluteUrl($params);
    }
    ...
    {
        $currentParams = Uni::$app->getRequest()->getQueryParams();
        $currentParams[0] = '/' . Uni::$app->controller->getRoute();
