...
use uni\db\ActiveRecordInterface;
use uni\validators\StringValidator;
use uni\web\Request;
use uni\base\Model;
...
class BaseHtml
{
     ...
     * The [[\uni\base\Application::charset|application charset]] will be used for encoding.
    ...
    {
        return htmlspecialchars($content, ENT_QUOTES | ENT_SUBSTITUTE, Uni::$app ? Uni::$app->charset : 'UTF-8', $doubleEncode);
    }
     ...
     * See [[\uni\web\Request::methodParam]] for more details.
     * @param array $options the tag options in terms of name-value pairs. These will be rendered as
     * the attributes of the resulting tag. The values will be HTML-encoded using [[encode()]].
     * If a value is null, the corresponding attribute will not be rendered.
     * See [[renderTagAttributes()]] for details on how attributes are being rendered.
    ...
    {
        $header = isset($options['header']) ? $options['header'] : '<p>' . Uni::t('uni', 'Please fix the following errors:') . '</p>';
     ...
     * the `maxlength` option will take the value of [[\uni\validators\StringValidator::max]].
     * @param Model $model the model object
     ...
     * if `'data' => ['id' => 1, 'name' => 'uni']`, then this will be rendered:
     * `data-id="1" data-name="uni"`.
     * Additionally `'data' => ['params' => ['id' => 1, 'name' => 'uni'], 'status' => 'ok']` will be rendered as:
     * `data-params='{"id":1,"name":"uni"}' data-status="ok"`.
     *
    ...
    public static function getAttributeValue($model, $attribute)
    {
        ...
        // https://github.com/unisoft/uni2/issues/1457
        if (is_array($value)) {
