...
class FileCache extends Cache
{
    ...
    {
        parent::init();
        $this->cachePath = Uni::getAlias($this->cachePath);
    ...
    protected function setValue($key, $value, $duration)
    {
        ...
        if (@file_put_contents($cacheFile, $value, LOCK_EX) !== false) {
            ...
            Uni::warning("Unable to write cache file '{$cacheFile}': {$error['message']}", __METHOD__);
            return false;
        }
    }
            ...
            while (($file = readdir($handle)) !== false) {
                ...
                if (is_dir($fullPath)) {
                    ...
                    if (!$expiredOnly) {
                        if (!@rmdir($fullPath)) {
                            $error = error_get_last();
                            Uni::warning("Unable to remove directory '{$fullPath}': {$error['message']}", __METHOD__);
                        }
                    }
                } elseif (!$expiredOnly || $expiredOnly && @filemtime($fullPath) < time()) {
                    if (!@unlink($fullPath)) {
                        $error = error_get_last();
                        Uni::warning("Unable to remove file '{$fullPath}': {$error['message']}", __METHOD__);
                    }
                }
            }
            closedir($handle);
