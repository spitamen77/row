 ...
 *
 * AssetManager is configured as an application component in [[\uni\web\Application]] by default.
 * You can access that instance via `Uni::$app->assetManager`.
 *
...
class AssetManager extends Component
{
     ...
     *
     * This is passed as a parameter `beforeCopy` to [[\uni\helpers\FileHelper::copyDirectory()]].
     */
    public $beforeCopy;
    /**
     ...
     * for [[beforeCopy]].
     * This is passed as a parameter `afterCopy` to [[\uni\helpers\FileHelper::copyDirectory()]].
     */
    public $afterCopy;
    /**
    ...
    {
        parent::init();
        $this->basePath = Uni::getAlias($this->basePath);
        ...
        $this->baseUrl = rtrim(Uni::getAlias($this->baseUrl), '/');
    }
    ...
    protected function loadBundle($name, $config = [], $publish = true)
    {
        ...
        $bundle = Uni::createObject($config);
        if ($publish) {
            $bundle->publish($this);
        }
        return $bundle;
    ...
    public function getAssetUrl($bundle, $asset)
    {
        if (($actualAsset = $this->resolveAsset($bundle, $asset)) !== false) {
            if (strncmp($actualAsset, '@web/', 5) === 0) {
                ...
                $basePath = Uni::getAlias('@webroot');
                $baseUrl = Uni::getAlias('@web');
            } else {
                $asset = Uni::getAlias($actualAsset);
    ...
    protected function resolveAsset($bundle, $asset)
    {
        ...
        $n = mb_strlen($asset, Uni::$app->charset);
        foreach ($this->assetMap as $from => $to) {
            $n2 = mb_strlen($from, Uni::$app->charset);
     ...
     * discussion: http://code.google.com/p/uni/issues/detail?id=2579
     *
    ...
    protected function hash($path)
    {
        ...
        return sprintf('%x', crc32($path . Uni::getVersion()));
    }
}
