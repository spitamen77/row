 ...
 *
 * Request is configured as an application component in [[\uni\web\Application]] by default.
 * You can access that instance via `Uni::$app->request`.
 *
 ...
 * @since alfa version
 */
class Request extends \uni\base\Request
{
    /**
     ...
     * When CSRF validation is enabled, forms submitted to an Uni Web application must be originated
     * from the same application. If not, a 400 HTTP exception will be raised.
     *
     * Note, this feature requires that the user client accepts cookie. Also, to use this feature,
     * forms submitted via POST method must contain a hidden input whose name is specified by [[csrfParam]].
     * You may use [[\uni\helpers\Html::beginForm()]] to generate his hidden input.
     *
     * In JavaScript, you may get the values of [[csrfParam]] and [[csrfToken]] via `uni.getCsrfParam()` and
     * `uni.getCsrfToken()`, respectively. The [[\uni\web\UniAsset]] asset must be registered.
     * You also need to include CSRF meta tags in your pages by using [[\uni\helpers\Html::csrfMetaTags()]].
     *
     ...
     * corresponding configurations for [[Uni::createObject|creating the parser objects]].
    ...
    {
        $result = Uni::$app->getUrlManager()->parseRequest($this);
        if ($result !== false) {
            ...
            throw new NotFoundHttpException(Uni::t('uni', 'Page not found.'));
        }
    }
     ...
     * @throws \uni\base\InvalidConfigException if a registered parser does not implement the [[RequestParserInterface]].
     * @see getMethod()
     * @see getBodyParam()
     * @see setBodyParams()
     */
    public function getBodyParams()
    {
        if ($this->_bodyParams === null) {
            ...
            if (isset($this->parsers[$contentType])) {
                $parser = Uni::createObject($this->parsers[$contentType]);
                if (!($parser instanceof RequestParserInterface)) {
                    throw new InvalidConfigException("The '$contentType' request parser is invalid. It must implement the uni\\web\\RequestParserInterface.");
                }
                $this->_bodyParams = $parser->parse($this->getRawBody(), $contentType);
            } elseif (isset($this->parsers['*'])) {
                $parser = Uni::createObject($this->parsers['*']);
                if (!($parser instanceof RequestParserInterface)) {
                    throw new InvalidConfigException("The fallback request parser is invalid. It must implement the uni\\web\\RequestParserInterface.");
                }
                $this->_bodyParams = $parser->parse($this->getRawBody(), $contentType);
     ...
     * @return string schema and hostname part (with port number if needed) of the request URL (e.g. `http://www.uniframework.com`)
     * @see setHostInfo()
     */
    ...
    protected function loadCookies()
    {
        ...
        if ($this->enableCookieValidation) {
            ...
            foreach ($_COOKIE as $name => $value) {
                ...
                $data = Uni::$app->getSecurity()->validateData($value, $this->cookieValidationKey);
                if ($data === false) {
                    continue;
                }
    ...
    protected function loadCsrfToken()
    {
        if ($this->enableCsrfCookie) {
            ...
            return Uni::$app->getSession()->get($this->csrfParam);
        }
    }
    ...
    {
        $token = Uni::$app->getSecurity()->generateRandomString();
        if ($this->enableCsrfCookie) {
            $cookie = $this->createCsrfCookie($token);
            Uni::$app->getResponse()->getCookies()->add($cookie);
        } else {
            Uni::$app->getSession()->set($this->csrfParam, $token);
        }
        return $token;
    }
