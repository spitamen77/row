 ...
 * @since alfa version
 */
class ErrorHandler extends \uni\base\ErrorHandler
{
    /**
     ...
     * using `Uni::$app->errorHandler->exception. This property defaults to null, meaning ErrorHandler
     ...
     */
    public $errorView = '@uni/views/errorHandler/error.php';
    /**
     * @var string the path of the view file for rendering exceptions.
     */
    public $exceptionView = '@uni/views/errorHandler/exception.php';
    /**
     * @var string the path of the view file for rendering exceptions and errors call stack element.
     */
    public $callStackItemView = '@uni/views/errorHandler/callStackItem.php';
    /**
     * @var string the path of the view file for rendering previous exceptions.
     */
    public $previousExceptionView = '@uni/views/errorHandler/previousException.php';
    /**
    ...
    protected function renderException($exception)
    {
        ...
        $useErrorView = $response->format === Response::FORMAT_HTML && (!UNI_DEBUG || $exception instanceof UserException);

        if ($useErrorView && $this->errorAction !== null) {
            $result = Uni::$app->runAction($this->errorAction);
            ...
            if (UNI_ENV_TEST || isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest') {
                // AJAX request
    ...
    {
        if (!UNI_DEBUG && !$exception instanceof UserException && !$exception instanceof HttpException) {
        ...
        if (UNI_DEBUG) {
            ...
            if (!$exception instanceof UserException) {
                ...
                if ($exception instanceof \uni\db\Exception) {
    ...
    {
        if (strpos($class, 'uni\\') !== 0) {
            return null;
        }

        $page = $this->htmlEncode(strtolower(str_replace('\\', '-', $class)));
        $url = "http://www.uniframework.com/doc-2.0/$page.html";
        if ($method) {
            $url .= "#$method()-detail";
        }
    ...
    public function renderFile($_file_, $_params_)
    {
        ...
        if ($this->exception instanceof ErrorException || !Uni::$app->has('view')) {
            ob_start();
            ob_implicit_flush(false);
            extract($_params_, EXTR_OVERWRITE);
            require(Uni::getAlias($_file_));

            return ob_get_clean();
        } else {
            return Uni::$app->getView()->renderFile($_file_, $_params_, $this);
        }
    }
    ...
    {
        return $file === null || strpos(realpath($file), UNI2_PATH . DIRECTORY_SEPARATOR) === 0;
    }
    ...
    {
        return '<a href="http://github.com/unibox/uni/" target="_blank">' . $this->htmlEncode(Uni::getVersion()) . '</a>';
    }
    ...
    {
        if ($exception instanceof \uni\base\Exception || $exception instanceof \uni\base\InvalidCallException || $exception instanceof \uni\base\InvalidParamException || $exception instanceof \uni\base\UnknownMethodException) {
