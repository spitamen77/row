...
use uni\base\InvalidValueException;
 ...
 *
 * User is configured as an application component in [[\uni\web\Application]] by default.
 * You can access that instance via `Uni::$app->user`.
 *
...
class User extends Component
{
     ...
     * For example, [[\uni\filters\auth\HttpBearerAuth]] will set this parameter to be `uni\filters\auth\HttpBearerAuth`.
    ...
    {
        $value = Uni::$app->getRequest()->getCookies()->getValue($this->identityCookie['name']);
        if ($value === null) {
            return;
        }
        ...
        if ($identity->validateAuthKey($authKey)) {
            if ($this->beforeLogin($identity, true, $duration)) {
                ...
                $ip = Uni::$app->getRequest()->getUserIP();
                Uni::info("User '$id' logged in from $ip via cookie.", __METHOD__);
                $this->afterLogin($identity, true, $duration);
            }
        } else {
            Uni::warning("Invalid auth key attempted for user '$id': $authKey", __METHOD__);
        }
    }
    ...
    public function logout($destroySession = true)
    {
        ...
        if ($identity !== null && $this->beforeLogout($identity)) {
            ...
            $id = $identity->getId();
            $ip = Uni::$app->getRequest()->getUserIP();
            Uni::info("User '$id' logged out from $ip.", __METHOD__);
            if ($destroySession && $this->enableSession) {
                Uni::$app->getSession()->destroy();
            }
    ...
    {
        $url = Uni::$app->getSession()->get($this->returnUrlParam, $defaultUrl);
        if (is_array($url)) {
            if (isset($url[0])) {
                return Uni::$app->getUrlManager()->createUrl($url);
            } else {
                $url = null;
            }
        }

        return $url === null ? Uni::$app->getHomeUrl() : $url;
    }
    ...
    {
        Uni::$app->getSession()->set($this->returnUrlParam, $url);
    }
    ...
    public function loginRequired($checkAjax = true)
    {
        ...
        if ($this->loginUrl !== null) {
            ...
            if ($loginUrl[0] !== Uni::$app->requestedRoute) {
                return Uni::$app->getResponse()->redirect($this->loginUrl);
            }
        }
        throw new ForbiddenHttpException(Uni::t('uni', 'Login Required'));
    }
    ...
    protected function renewIdentityCookie()
    {
        ...
        $value = Uni::$app->getRequest()->getCookies()->getValue($name);
        if ($value !== null) {
            $data = json_decode($value, true);
    ...
    public function switchIdentity($identity, $duration = 0)
    {
        ...
        if ($identity) {
            ...
            Uni::$app->getResponse()->getCookies()->remove(new Cookie($this->identityCookie));
        }
    }
     ...
     * [[\uni\rbac\ManagerInterface::checkAccess()]] to obtain the up-to-date access result. Note that this
     ...
     * @return \uni\rbac\ManagerInterface
     * @since alfa version.6
     */
    protected function getAuthManager()
    {
        return Uni::$app->getAuthManager();
    }
}
