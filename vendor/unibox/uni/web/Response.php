 ...
 *
 * Response is configured as an application component in [[\uni\web\Application]] by default.
 * You can access that instance via `Uni::$app->response`.
 *
 ...
 * ```php
 * 'response' => [
 *     'format' => uni\web\Response::FORMAT_JSON,
 *     'charset' => 'UTF-8',
 *     // ...
 * ]
 * ```
 ...
 * @since alfa version
 */
class Response extends \uni\base\Response
{
    /**
     ...
     * This property is mainly set by [[\uni\filters\ContentNegotiator]].
     */
    ...
    protected function sendCookies()
    {
        ...
        foreach ($this->getCookies() as $cookie) {
            ...
            if ($cookie->expire != 1  && isset($validationKey)) {
                $value = Uni::$app->getSecurity()->hashData(serialize([$cookie->name, $value]), $validationKey);
            }
     ...
     *
     * ```php
     * Uni::$app->response->xSendFile('/home/user/Pictures/picture1.jpg');
     * ```
     *
     ...
     *
     * ```php
     * return Uni::$app->getResponse()->redirect($url);
     * ```
     *
     ...
     *
     * ```php
     * Uni::$app->getResponse()->redirect($url)->send();
     * return;
     * ```
     *
     ...
     *
     * If you use the "uni" JavaScript module, it will handle the AJAX redirection as
    ...
    public function redirect($url, $statusCode = 302, $checkAjax = true)
    {
        ...
        if (strpos($url, '/') === 0 && strpos($url, '//') !== 0) {
            $url = Uni::$app->getRequest()->getHostInfo() . $url;
        }

        if ($checkAjax) {
            if (Uni::$app->getRequest()->getIsPjax()) {
                $this->getHeaders()->set('X-Pjax-Url', $url);
            } elseif (Uni::$app->getRequest()->getIsAjax()) {
     ...
     *
     * ```php
     * return Uni::$app->getResponse()->refresh();
     * ```
     *
    ...
    {
        return $this->redirect(Uni::$app->getRequest()->getUrl() . $anchor);
    }
    ...
    {
        return [
            self::FORMAT_HTML => 'uni\web\HtmlResponseFormatter',
            self::FORMAT_XML => 'uni\web\XmlResponseFormatter',
            self::FORMAT_JSON => 'uni\web\JsonResponseFormatter',
            self::FORMAT_JSONP => [
    ...
    protected function prepare()
    {
        ...
        if (isset($this->formatters[$this->format])) {
            ...
            if (!is_object($formatter)) {
                $this->formatters[$this->format] = $formatter = Uni::createObject($formatter);
            }
