 ...
 *
 * UrlManager is configured as an application component in [[\uni\base\Application]] by default.
 * You can access that instance via `Uni::$app->urlManager`.
 *
...
class UrlManager extends Component
{
     ...
     * and can thus produce more user-friendly URLs, such as "/news/Uni-is-released", instead of
     ...
     */
    public $ruleConfig = ['class' => 'uni\web\UrlRule'];
    ...
    public function init()
    {
        ...
        if (is_string($this->cache)) {
            $this->cache = Uni::$app->get($this->cache, false);
        }
    ...
    protected function buildRules($rules)
    {
        ...
        foreach ($rules as $key => $rule) {
            ...
            if (is_array($rule)) {
                $rule = Uni::createObject(array_merge($this->ruleConfig, $rule));
            }
    ...
    public function parseRequest($request)
    {
        if ($this->enablePrettyUrl) {
            ...
            Uni::trace('No matching URL rules. Using default URL parsing logic.', __METHOD__);
            ...
            Uni::trace('Pretty URL not enabled. Using default URL parsing logic.', __METHOD__);
     ...
     *
     * Note that unlike [[\uni\helpers\Url::toRoute()]], this method always treats the given route
    ...
    public function getHostInfo()
    {
        if ($this->_hostInfo === null) {
            $request = Uni::$app->getRequest();
            if ($request instanceof \uni\web\Request) {
