...
class ActionColumn extends Column
{
     ...
     *
     * ```
     * ['class' => 'uni\grid\ActionColumn', 'template' => '{view} {update}'],
     * ```
     *
     * @see buttons
     */
    ...
    protected function initDefaultButtons()
    {
        if (!isset($this->buttons['view'])) {
            $this->buttons['view'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Uni::t('uni', 'View'),
                    'aria-label' => Uni::t('uni', 'View'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
        ...
        if (!isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Uni::t('uni', 'Update'),
                    'aria-label' => Uni::t('uni', 'Update'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
        ...
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Uni::t('uni', 'Delete'),
                    'aria-label' => Uni::t('uni', 'Delete'),
                    'data-confirm' => Uni::t('uni', 'Are you sure you want to delete this item?'),
                    'data-method' => 'post',
                    'data-pjax' => '0',
                ], $this->buttonOptions);
     ...
     * @param \uni\db\ActiveRecord $model the data model
