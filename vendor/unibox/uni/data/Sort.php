 ...
 * passed to [[\uni\db\Query::orderBy()]] to construct a DB query. This property is read-only.
 *
...
class Sort extends BaseObject
{
    ...
    public $params;
    /**
     * @var \uni\web\UrlManager the URL manager used for creating sort URLs. If not set,
     ...
     * This can be passed to [[\uni\db\Query::orderBy()]] to construct a DB query.
     */
     ...
     * If no label is defined, [[\uni\helpers\Inflector::camel2words()]] will be called to get a label.
     * Note that it will not be HTML-encoded.
    ...
    public function createUrl($attribute, $absolute = false)
    {
        ...
        $params[0] = $this->route === null ? Uni::$app->controller->getRoute() : $this->route;
        $urlManager = $this->urlManager === null ? Uni::$app->getUrlManager() : $this->urlManager;
        if ($absolute) {
