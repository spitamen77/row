...
use app\components\Model as M;
use Uni;
use uni\helpers\FileHelper;
use uni\widgets\Block;
use uni\widgets\ContentDecorator;
use uni\widgets\FragmentCache;
...
class View extends Component
{
     ...
     * ```php
     * [
     *     'tpl' => ['class' => 'uni\smarty\ViewRenderer'],
     *     'twig' => ['class' => 'uni\twig\ViewRenderer'],
     * ]
     * ```
     *
    ...
    public function init()
    {
        ...
        if (is_array($this->theme)) {
            if (!isset($this->theme['class'])) {
                $this->theme['class'] = 'uni\base\Theme';
            }
            $this->theme = Uni::createObject($this->theme);
     ...
     * @param BaseObject $context the context to be assigned to the view and can later be accessed via [[context]]
    ...
    protected function findViewFile($view, $context = null)
    {
        if (strncmp($view, '@', 1) === 0) {
            // e.g. "@app/views/main"
            $file = Uni::getAlias($view);
        } elseif (strncmp($view, '//', 2) === 0) {
            // e.g. "//layouts/main"
            $file = Uni::$app->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
        } elseif (strncmp($view, '/', 1) === 0) {
            // e.g. "/site/index"
            if (Uni::$app->controller !== null) {
                $file = Uni::$app->controller->module->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
            } else {
     ...
     * @param array $params the parameters (name-value pairs) that will be extracted and made available in the view file.
     * @param BaseObject $context the context that the view should use for rendering the view. If null,
    ...
    {
        $viewFile = Uni::getAlias($viewFile);
        ...
        if ($this->beforeRender($viewFile, $params)) {
            Uni::trace("Rendering view file: $viewFile", __METHOD__);
            $ext = pathinfo($viewFile, PATHINFO_EXTENSION);
            if (isset($this->renderers[$ext])) {
                if (is_array($this->renderers[$ext]) || is_string($this->renderers[$ext])) {
                    $this->renderers[$ext] = Uni::createObject($this->renderers[$ext]);
                }
    ...
    public function renderDynamic($statements)
    {
        if (!empty($this->cacheStack)) {
            ...
            $placeholder = "<![CDATA[UNI-DYNAMIC-$n]]>";
