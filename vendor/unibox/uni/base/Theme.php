...
class Theme extends Component
{
    ...
    {
        $this->_baseUrl = rtrim(Uni::getAlias($url), '/');
    }
    ...
    {
        $this->_basePath = Uni::getAlias($path);
    }
    ...
    public function applyTo($path)
    {
        ...
        if (empty($pathMap)) {
            ...
            $pathMap = [Uni::$app->getBasePath() => [$basePath]];
        }

        $path = FileHelper::normalizePath($path);

        foreach ($pathMap as $from => $tos) {
            $from = FileHelper::normalizePath(Uni::getAlias($from)) . DIRECTORY_SEPARATOR;
            if (strpos($path, $from) === 0) {
                $n = strlen($from);
                foreach ((array) $tos as $to) {
                    $to = FileHelper::normalizePath(Uni::getAlias($to)) . DIRECTORY_SEPARATOR;
