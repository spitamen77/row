 ...
 *
 * ```php
 * $container = new \uni\di\Container;
 * $container->set('cache', 'uni\caching\DbCache', Instance::of('db'));
 * $container->set('db', [
 ...
 *     {
 *         parent::init();
 *         $this->db = Instance::ensure($this->db, 'uni\db\Connection');
 *     }
 * }
 * ```
 *
...
class Instance
{
     ...
     *
     * If you do not specify a container, the method will first try `Uni::$app` followed by `Uni::$container`.
     *
     * For example,
     *
     * ```php
     ...
     * If null, the method will first try `Uni::$app` then `Uni::$container`.
    ...
    public function get($container = null)
    {
        ...
        if (Uni::$app && Uni::$app->has($this->id)) {
            return Uni::$app->get($this->id);
        } else {
            return Uni::$container->get($this->id);
        }
    }
}
