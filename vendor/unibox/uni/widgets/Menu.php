 ...
 *         ]],
 *         ['label' => 'Login', 'url' => ['site/login'], 'visible' => Uni::$app->user->isGuest],
 *     ],
 * ]);
 * ```
 *
...
class Menu extends Widget
{
    ...
    public function run()
    {
        if ($this->route === null && Uni::$app->controller !== null) {
            $this->route = Uni::$app->controller->getRoute();
        }
        if ($this->params === null) {
            $this->params = Uni::$app->request->getQueryParams();
        }
    ...
    protected function isItemActive($item)
    {
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            $route = Uni::getAlias($item['url'][0]);
            if ($route[0] !== '/' && Uni::$app->controller) {
                $route = Uni::$app->controller->module->getUniqueId() . '/' . $route;
            }
