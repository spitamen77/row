 ...
 * 'controllerMap' => [
 *     'fixture' => [
 *         'class' => 'uni\faker\FixtureController',
 *     ],
 * ],
 * ~~~
 *
 ...
 *     'city' => $faker->city,
 *     'password' => Uni::$app->getSecurity()->generatePasswordHash('password_' . $index),
 *     'auth_key' => Uni::$app->getSecurity()->generateRandomString(),
 ...
 *
 * ~~~
 * uni fixture/generate user
 *
 * //generate fixtures from several templates, for example:
 * uni fixture/generate user profile team
 * ~~~
 *
 ...
 *
 * ~~~
 * uni fixture/generate-all --count=3
 * ~~~
 *
 ...
 * uni fixture/generate user --count=5 --language=ru_RU
 *
 * //read templates from the other path
 * uni fixture/generate-all --templatePath=@app/path/to/my/custom/templates
 *
 * //generate fixtures into other folders
 * uni fixture/generate-all --fixtureDataPath=@tests/unit/fixtures/subfolder1/subfolder2/subfolder3
 * ~~~
 *
 ...
 * uni fixture/templates
 *
 * //list all templates under specified template path
 * uni fixture/templates --templatePath='@app/path/to/my/custom/templates'
 * ~~~
 *
 ...
 * @since alfa version.0
 */
class FixtureController extends \uni\console\controllers\FixtureController
{
    /**
     ...
     * ~~~
     * 
     * @throws \uni\base\InvalidParamException
     * @throws \uni\console\Exception
     */
    public function actionGenerate()
    {
        ...
        $templatePath = Uni::getAlias($this->templatePath);
        $fixtureDataPath = Uni::getAlias($this->fixtureDataPath);
    ...
    private function notifyNoTemplatesFound()
    {
        ...
        $this->stdout("\t " . Uni::getAlias($this->templatePath) . " \n\n", Console::FG_GREEN);
    }
    ...
    private function findTemplatesFiles(array $templatesNames = [])
    {
        ...
        if ($findAll) {
            $files = FileHelper::findFiles(Uni::getAlias($this->templatePath), ['only' => ['*.php']]);
        } else {
            $filesToSearch = [];
            ...
            $files = FileHelper::findFiles(Uni::getAlias($this->templatePath), ['only' => $filesToSearch]);
        }
    ...
    public function getGenerator()
    {
        if ($this->_generator === null) {
            $language = $this->language === null ? Uni::$app->language : $this->language;
    ...
    {
        $path = Uni::getAlias($this->templatePath, false);
    ...
    public function confirmGeneration($files)
    {
        ...
        $this->stdout("\t" . Uni::getAlias($this->fixtureDataPath) . "\n\n", Console::FG_GREEN);
