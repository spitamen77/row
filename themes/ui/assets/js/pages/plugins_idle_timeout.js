$(function() {
    ...
    $body.append('<div class="uk-modal" id="modal_idle">' +
        ...
        '</div>' +
        '</div>');
    if (document.location.pathname != "/users/auth/login") {
        var modal = UIkit.modal("#modal_idle", {
            ...
            session = {
                ...
                minWarning: 30000,           //(ms) If they come back to page (on mobile), The minumum amount, before we just log them out
        ...
        session = {
            //Logout Settings
            inactiveTimeout: 500000,      //(ms) The time until we display a warning message
            warningTimeout: 500000,      //(ms) The time until we log them out
            minWarning: 500000,           //(ms) If they come back to page (on mobile), The minumum amount, before we just log them out
            warningStart: null,         //Date time the warning was started
            warningTimer: null,         //Timer running every second to countdown to logout
