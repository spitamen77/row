$(function() {
    // add note
    unibox_sticky_notes.add_note();
    // note actions
    unibox_sticky_notes.actions();
    // load sticky notes
    unibox_sticky_notes.load_notes();
...
var $note_template = $('#note_template').html(),
    ...
    // notes color picker
    notesColorPicker = unibox_helpers.color_picker($('<div id="notes_colors_wrapper"></div>'),pallete,changeBg).prop('outerHTML');
    ...
    add_note: function() {
        ...
        $form.on('click', '#note_add', function(e) {
            ...
            if(title == '') {
                ...
                unibox_md.update_input($title_input);
                return;
            }
            if(content == '') {
                $content_input.addClass('md-input-danger');
                unibox_md.update_input($content_input);
                return;
            }
            ...
            if(title && content) {
                ...
                // initialize iCheck
                unibox_md.checkbox_radio($grid.find('[data-md-icheck]'));
        ...
        var addChecklist_item = function() {
            ...
            if(title != '') {
                ...
                $title_input.val('');
                unibox_md.checkbox_radio($list.find('[data-md-icheck]'));
            }
        };
